                ...
                if (formItems != null && formItems.size() > 0) {
                    ...
                    for (FileItem item : formItems) {
                        ...
                        if (!item.isFormField()) {
                            ...
                            String jobName=fN.substring(0,fN.length()-17);
...
//                            String filePath = uploadPath + File.separator + fileName;
                            ...
                            Submission currentSub = ss.subTest(id, testID, testName, filePath,
                                    TaskMonitor.SUBMITTED_STATE, "等待运行",jobName);
                            System.out.println("上传到服务器成功，当前任务时："+currentSub);
                            // 任务调度入口c
                            {
                                System.out.println("uploaded to server");
                                // 提交jar到集群
                                RemoteUploadImpl rp = ac.getBean("uploader", RemoteUploadImpl.class);
                                rp.upload(currentSub.getURI(), REMOTE_URL);
